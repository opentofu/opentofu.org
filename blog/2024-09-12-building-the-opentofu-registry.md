...
- For providers, it would is located at `/v1/providers/NAMESPACE/NAME/VERSION/download/OS/ARCH` ([example]( https://registry.opentofu.org/v1/providers/opentofu/random/2.0.0/download/linux/amd64)).
...
Doing all this work on the backend and splitting the display logic from the data also had an unintended but very welcome side effect: we were able to offer [an API for provider and module documentation](https://api.opentofu.org), which would come in handy as just a few weeks later [Jetbrains requested just such an API for their OpenTofu integration](https://youtrack.jetbrains.com/issue/IJPL-157423/Support-for-OpenTofu-in-Terraform-and-HCL-Plugin#focus=Comments-27-10245678.0-0).
